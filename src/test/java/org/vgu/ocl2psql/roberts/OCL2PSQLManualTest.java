...
//                "Module_Period.allInstances() ->forAll(mp|mp.ends < mp.period.ends and mp.starts > mp.period.starts)"
//                "Module.allInstances() ->forAll(m| m.module_periods ->forAll(mp|mp.starts < @SQL(CURDATE())))",
//                "Record.allInstances() ->collect(r| r.module_period.module)",
//                "University.allInstances()->collect(u|u.programs->collect(p|p.modules)->flatten())",
//                "Module.allInstances()->collect(m|m.module_group.group_name)->isUnique()",
//                "Program.allInstances()->collect(p|p.modules->collect(m|m.module_group.group_name))->flatten()->isUnique()",
//                "Module.allInstances()->collect(m|m.module_group.group_name)",
