...
public class SetOperationList implements SelectBody {
	...
	public void accept(Injector injector) {
		List<SelectBody> selectsBody = new ArrayList<SelectBody>();
		for(SelectBody selectBody : this.getSelects()) {
			Injector recInjector = new Injector();
			recInjector.setAction(injector.getAction());
			recInjector.setContext(injector.getContext());
			recInjector.setPolicy(injector.getPolicy());
			selectBody.accept(recInjector);
			selectsBody.add(recInjector.getResult());	
		}
		this.selects = selectsBody;
		injector.setResult(this);
	}
