...
public class Function extends ASTNodeAccessImpl implements Expression {
	...
	public void accept(InjectorExpression injectorExpression) {
		...
		String funName = this.getName();
		Function fun = new Function();
		ExpressionList list = new ExpressionList();
		//list.setExpressions(new ArrayList<Expression>());
		fun.setParameters(list);
		fun.setName(funName);
		
		//
		List<Expression> pars = new ArrayList<Expression>();
		if (this.getParameters() != null) {
		for(Expression exp : this.getParameters().getExpressions()) {
			exp.accept(injectorExpression);
			pars.add(injectorExpression.getResult());
		}
		fun.getParameters().setExpressions(pars);
		} else {
			fun.setParameters(null);
		}
		injectorExpression.setResult(fun);
